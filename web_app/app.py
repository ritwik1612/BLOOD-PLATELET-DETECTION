...
BPD_ROOT = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parents[1]))
sys.path.insert(0, str(BPD_ROOT))

from inference.pipeline import default_pipeline
from utils.config import PROJECT_ROOT
from utils.image_quality import assess_smear_image
...
RUNTIME_ROOT = Path(tempfile.gettempdir()) / "bpd-open-set" if (RUNNING_ON_VERCEL or getattr(sys, "frozen", False)) else APP_ROOT / "static"
...
def predict():
    ...
    try:
        raw_image = cv2.imread(str(upload_path))
        quality = assess_smear_image(raw_image)
        if quality["status"] == "reject":
            return jsonify({"error": quality["message"], "quality": quality}), 422
